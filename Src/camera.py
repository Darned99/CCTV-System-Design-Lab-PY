...
# Ścieżka do folderu Videos
videos_dir = os.path.join(os.path.dirname(__file__), '../Videos')

# Tworzenie folderu Videos, jeśli nie istnieje
...
out2 = cv2.VideoWriter(os.path.join(videos_dir, "video.mp4"), fourcc, 20.0, frame_size)
...
while True:
    ...
    if len(faces1) + len(bodies1) > 0:
        if detection:
            ...
            out1 = cv2.VideoWriter(os.path.join(videos_dir, f"{current_time}.mp4"), fourcc, 20.0, frame_size)
    ...
    for (x, y, width, height) in faces2:
        cv2.rectangle(frame2, (x, y), (x + width, y + height), (255, 0, 0), 3)
